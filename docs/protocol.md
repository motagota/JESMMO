...
| `road.cancel` | C→S | `order_id` — remove a **pristine** (open, zero-progress) road plan outright (#106). Anything with contributed stone is refused toward demolition — hauled stone is never vaporised. Ack `road.cancelled {order_id}` | **live** |
| `road.demolish` | C→S | `order_id` — post a **demolition order** for a built road or part-built plan (#106): kind `demo_<target id>` (the link + double-demolition guard), requires `{tool_kit: 1}`, carries the road's path (on-site contribution) but **no placement** (completion never spawns a structure). An open target flips to state `demolishing` (contributions freeze, its stakes drop); a built road keeps rendering until the job's done. Ack `road.demolition_planned {order_id, demo_order_id}` | **live** |

Completing a demolition order (an ordinary `build.contribute` of the tool kit) removes the target road everywhere — the order row goes, the built road's `structure_*` entity is `despawn`ed to every client — and **refunds the banked stone** (a built road: its full required cost; a part-built plan: its contributed progress) **pro-rata to the demolition's contributors, into their town storage** (`store.update` pushed to online recipients; storage sidesteps the carry cap).
