...
writes `artifacts/world_v3/` (the production capital bake: real Brisbane DEM at native 5m resolution, 1600 tiles over the near-full 25.6km extent — see `terrain.toml`'s own comments). `--stage <ingest|water|stylize|detail|erode|classify|export>` runs a single stage; `--debug-dump <dir>` writes a hillshade/water-mask/biome-map PNG at every stage boundary; `--force` bypasses the per-stage cache.
