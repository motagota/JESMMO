...
##
## Custom-drawn (`_draw()`) rather than stock nodes: this is the first place
## in the client that needs many small, cheaply-updated markers projected
## from world coordinates we already have, which doesn't warrant a live
## top-down `SubViewport`/camera render.
class_name Minimap
extends Control
...
var _player_pos := Vector2(3200, 3200)
var _player_facing := Vector2(0, -1)
var _plots: Array = []
var _my_plot_id := ""
var _has_home := false
var _home_pos := Vector2.ZERO
var _district_bounds: Dictionary = {}

func _ready() -> void:
	set_anchors_and_offsets_preset(Control.PRESET_TOP_RIGHT, Control.PRESET_MODE_KEEP_SIZE, int(_MARGIN))
	custom_minimum_size = Vector2(_SIZE, _SIZE)
	size = Vector2(_SIZE, _SIZE)
	clip_contents = true # so anything projected outside the widget is cropped for free
	mouse_filter = Control.MOUSE_FILTER_IGNORE

func set_player(wx: float, wy: float, facing: Vector2) -> void:
	_player_pos = Vector2(wx, wy)
...
func _project(wp: Vector2) -> Vector2:
	...
	return size * 0.5 + (wp - _player_pos) * scale

func _draw() -> void:
	draw_rect(Rect2(Vector2.ZERO, size), Color(0.05, 0.05, 0.06, 0.55))

	if not _district_bounds.is_empty():
		var x0 := float(_district_bounds.get("x0", 0))
		var y0 := float(_district_bounds.get("y0", 0))
		var x1 := float(_district_bounds.get("x1", 0))
		var y1 := float(_district_bounds.get("y1", 0))
	...
	for entry_v in _plots:
		...
		var mine := String(p.get("plot_id", "")) == _my_plot_id
	...
	# Home arrow at the edge, only when the player's own plot has scrolled out
	# of the visible radius (otherwise it's already shown as a gold plot square).
	if _has_home:
		var delta := _home_pos - _player_pos
		if delta.length() > _RADIUS_WORLD:
			var dir := delta.normalized()
			var edge := size * 0.5 + dir * (_SIZE * 0.5 - 10.0)
			_draw_marker(edge, dir, _COLOR_MINE)

	_draw_marker(size * 0.5, _player_facing, _COLOR_PLAYER)

## A small triangle at `pos` pointing along `dir` (already in the same
## world-aligned (x,y) space as everything else this widget projects, so no
## extra rotation math is needed — just vector arithmetic).
